...
class FftStream(Elaboratable):
    ...
    def elaborate(self, platform):
        m = Module()
        m.submodules.fft = mfft = FFT(size=self.nfft, i_width=self.width, o_width=self.width, m_width=self.width)

        cnt_fill = Signal(range(self.nfft))
        cnt_empty = Signal(range(self.nfft//2))
        m.d.comb += [
            mfft.i.addr.eq(cnt_fill),
            mfft.i.data.eq(self.sink.data),
            mfft.o.addr.eq(cnt_empty),
            self.source.data_r.eq(mfft.o.data.real),
            self.source.data_i.eq(mfft.o.data.imag),
        ]

        with m.FSM() as fsm:
            with m.State("FILL"):
                m.d.comb += [
                    mfft.i.en.eq(self.sink.valid),
